...
from utils.parsing_utils import clean_day_suffix, load_seen_ids, generate_pdf_attachment_check 
...
def scrape_meeting_metadata(mid, cid="144", session=None):
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"❌ Failed to fetch committee page for meeting {mid}")
            return {
        ...
        print(f"✅ Success: Scraped meeting {mid}")
        return {
            "web_meeting_code": str(mid),
            "meeting_id": f"kent_cc_{mid}",
            "scrape_timestamp": timestamp,
        ...
        print(f"❌ Exception occurred for meeting {mid}: {e}")
        return {
            "web_meeting_code": str(mid),
            "meeting_id": f"kent_cc_{mid}",
            "scrape_timestamp": timestamp,
                ...
                try:
                    ...
                    if data and not data.get("error"):
                        try:
                            ...
                            print(f"    ⚠️ Skipped PDF attachment check for meeting {data.get('meeting_id')} due to error: {e}")
